...
module.exports = class {
    ...
    async run(member) {
        ...
        if(inviterDataX.inviter) {
            ...
            this.client.channels.cache.get("916967873895886868").send(`<a:bothac:916968184798662716> <@${inviterDataX.inviter}> tarafından davet edilen \`${member.user.tag}\` sunucudan çıktı. (<@${inviterDataX.inviter}> \`${regular + bonus + fakecount}\` toplam davete sahip oldu.)`)
        } else {
            this.client.channels.cache.get("916967873895886868").send(`<a:bothac:916968184798662716> \`${member.user.tag}\` sunucudan çıkış yaptı. Veritabanında kullanıcıyı davet eden üye bulunamadı!`)
        }
    }
}