    ...
    async run(message, args, perm) {
       if(!message.member.roles.cache.has(Permissions.Ban.Auth_Roles) && !message.member.hasPermission("VIEW_AUDIT_LOG")) return
        ...
        const embed = new Discord.MessageEmbed()
            ...
            .setColor("RANDOM")
            .setDescription(`**${user.tag}** kullanıcısı **${message.author.tag}** tarafından başarıyla sunucudan yasaklandı. (Ceza Numarası: \`#${Root + 1}\`)`).setImage(Permissions.Ban.GIF.length > 0 ? Permissions.Ban.GIF : null);
