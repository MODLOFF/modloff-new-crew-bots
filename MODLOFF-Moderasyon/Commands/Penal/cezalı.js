        ...
        if (!message.guild.members.cache.has(user.id)) {
            ...
            await cezalar.findOne({ user: user.id }, async (err, doc) => {
                ...
                member.roles.set(member.roles.cache.has(Role.Jail.Role) ? [Role.Jail.Role, Role.Jail.Role] : [Role.Jail.Role]).catch(e => console.log(e))
                           ...
                           .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                           .setDescription(`${member} kişisine <@&${Role.Jail.Role}> rolü verildi. (Ceza Numarası: \`#${Root + 1}\`)`)
                           .setColor("RANDOM")
                       //  .setFooter(`${moment(Date.now()).format("LLL")}`)
                           message.channel.send(Rootn)
                           // let puan = await this.client.punishPoint(user.id)
                            await this.client.channels.cache.get(Log.Cezapuan_Log).send(`${member}; adlı üye aldığınız **#${Root + 1}** ID'li ceza ile **${await this.client.punishPoint(member.id) + 15}** ulaştınız.`).catch(e => { })
