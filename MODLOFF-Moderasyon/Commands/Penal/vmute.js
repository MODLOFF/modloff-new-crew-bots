...
class VMute extends Command {
    ...
    async run(message, args, perm) {
      ...
      //  if (message.member.roles.highest.position <= message.guild.members.cache.get(user.id).roles.highest.position) return this.client.yolla("Kendi rolünden yüksek kişilere işlem uygulayamazsın!", message.author, message.channel)
        ...
        await message.channel.send(`${Others.Emojis.Off_Mic} <@${user.id}> kişisi ${await this.client.turkishDate(time)} boyunca ses kanallarında susturuldu. (Ceza Numarası: \`#${Root + 1}\`)`)
        //let puan = await this.client.punishPoint(user.id)
        await this.client.channels.cache.get(Log.Cezapuan_Log).send(`${user}; adlı üye aldığınız **#${Root + 1}** ID'li ceza ile **${await this.client.punishPoint(user.id) + 10}** ulaştınız.`).catch(e => { })
        const mutelendı = new Discord.MessageEmbed()
            .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
            .setColor("32CD32")
            .setFooter(`Ceza Numarası: #${Root + 1}`)
        ...
        await wmute.findOne({user: user.id}, async(err,res) => {
            if(!res) {
                await message.channel.send(`${Others.Emojis.Off_Mic} <@${user.id}> kişisinin ${await this.client.turkishDate(time)} sürelik ses mutesi başlatılamadı kullanıcı sese bağlanınca otomatik olarak cezası başlayacak. (Ceza Numarası: \`#${Root + 1}\`)`)
