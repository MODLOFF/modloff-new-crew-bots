    ...
    async run(message, args, perm) {
        ...
        uyarılar.findOne({user: user.id}, async(err,res) => {
            if(!res) {
                ...
                await this.client.channels.cache.get(Log.Cezapuan_Log).send(`${user}; adlı üye aldığınız **#${Root + 1}** ID'li ceza ile **${await this.client.punishPoint(user.id) + 3}** ulaştınız.`).catch(e => { })
