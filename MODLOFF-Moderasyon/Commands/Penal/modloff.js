...
class Ban extends Command {
    ...
    async run(message, args, perm) {
       if(!message.member.roles.cache.has(Permissions.MODLOFF)) return
        if (args.length < 1) return this.client.yolla("Sikmek istediğin üyeyi etiketle ve tekrardan dene!", message.author, message.channel)
        ...
        //let Root = count ? count++ : 1;
        let Root = count
        const embed = new Discord.MessageEmbed()
            .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
            .setColor("RANDOM")
            .setDescription(`**${user.tag}** kullanıcısı **${message.author.tag}** tarafından başarıyla sunucudan sikildi. (Ceza Numarası: \`#${Root + 1}\`)`).setImage(Permissions.Ban.GIF.length > 0 ? Permissions.Ban.GIF : null);
...
Ban Sebebi: \`${reason}\`
            ...
            await this.client.channels.cache.get(Log.Ban.Log).send(` ${this.client.ok} **${user.tag}** adlı kullanıcı __${reason}__ sebebiyle **${message.author.tag}** tarafından sikildi.`).catch(e => { })
            await user.send(new Discord.MessageEmbed().setColor("RANDOM").setImage(Permissions.Ban.GIF.length > 0 ? Permissions.Ban.GIF : null).setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true })).setDescription(`
                **${message.guild.name}** sunucusundan ${reason} sebebiyle **${message.author.tag}** tarafından yasaklandın. (Ceza Numaran: \`#${Root + 1}\`)
           ` ))
