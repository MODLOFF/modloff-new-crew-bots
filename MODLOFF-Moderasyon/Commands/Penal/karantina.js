    ...
    async run(message, args, level) {
      ...
      /*  if (!message.guild.members.cache.has(user.id)) {
            const embedx = new Discord.MessageEmbed()
                ...
                .setColor("RANDOM")
                .setDescription(`${user.tag} kullanıcısı sunucuda bulunmamasına rağmen cezalıya atıldı.Sunucuya girişi engellendi. (Ceza Numarası: \`#${Root + 1}\`)`)
            ...
            await cezalar.findOne({ user: user.id }, async (err, doc) => {
                ...
                member.roles.set(member.roles.cache.has(Role.Jail.Karantina) ? [Role.Jail.Karantina, Role.Jail.Karantina] : [Role.Jail.Karantina]).catch(e => console.log(e))
                           const Rootn = new Discord.MessageEmbed()
                           .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                           .setDescription(`${member} kişisine <@&${Role.Jail.Karantina}> rolü verildi. (Ceza Numarası: \`#${Root + 1}\`)`)
                           .setColor("RANDOM")
                       //  .setFooter(`${moment(Date.now()).format("LLL")}`)
                           message.channel.send(Rootn)
                           // let puan = await this.client.punishPoint(user.id)
                            await this.client.channels.cache.get(Log.Cezapuan_Log).send(`${member}; adlı üye aldığınız **#${Root + 1}** ID'li ceza ile **${await this.client.punishPoint(member.id) + 20}** ulaştınız.`).catch(e => { })
