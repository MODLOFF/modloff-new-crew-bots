...
class Rolyonet extends Command {
    ...
    async run(message, args, level) {
        ...
        if (!args[1]) return this.client.yolla("Bir üye etiketle ve tekrardan dene!", message.author, message.channel)
        ...
        if (banNum == Limit.Role_Limit) return this.client.yolla("Gün içerisinde çok fazla rol işlemi uyguladığınız için komut geçici olarak kullanımınıza kapatılmıştır.", message.author, message.channel)
        if (args[0] == "al") {
